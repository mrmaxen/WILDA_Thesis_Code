 ...
 *
 *
 *	Slightly Edited by Max Nilsson for use with an MSP432
 */ 
...
int dwm_pos_set(dwm_pos_t* pos)
{
   ...
   *(uint32_t*)(tx_data+tx_len) = pos->z;
   ...
   msp_uart_tx(tx_data, &tx_len);      
   return msp_uart_rx(rx_data, &rx_len, 3);
}
...
int dwm_pos_get(dwm_pos_t* p_pos)
{
   ...
   tx_data[tx_len++] = 0;   
   msp_uart_tx(tx_data, &tx_len);   
   if(msp_uart_rx(rx_data, &rx_len, 18) == RV_OK)
   {
      data_cnt = 5;
...
int dwm_upd_rate_get(uint16_t *ur, uint16_t *ur_static)
{
   ...
   tx_data[tx_len++] = 0;   
   msp_uart_tx(tx_data, &tx_len);   
   if(msp_uart_rx(rx_data, &rx_len, 9) == RV_OK)
   {
      data_cnt = 5;
...
int dwm_cfg_get(dwm_cfg_t* cfg)
{
   ...
   tx_data[tx_len++] = 0;   
   msp_uart_tx(tx_data, &tx_len);   
   if(msp_uart_rx(rx_data, &rx_len, 7) == RV_OK)
   {
...
{ 
   ...
   tx_data[tx_len++] = 0;   
   msp_uart_tx(tx_data, &tx_len);   
   if(msp_uart_rx(rx_data, &rx_len, DWM1001_TLV_MAX_SIZE) == RV_OK)
   {
...
int dwm_ver_get(dwm_ver_t* ver)
{
   ...
   tx_data[tx_len++] = 0;   
   msp_uart_tx(tx_data, &tx_len);   
   if(msp_uart_rx(rx_data, &rx_len, 21) == RV_OK)
   {
      data_cnt = 3;      
      // read fw_version
...
int dwm_status_get(dwm_status_t* p_status)
{
   ...
   tx_data[tx_len++] = 0;   
   msp_uart_tx(tx_data, &tx_len);   
   if(msp_uart_rx(rx_data, &rx_len, 6) == RV_OK)
   {
